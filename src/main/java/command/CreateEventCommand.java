...
/**
 * {@link CreateEventCommand} allows {@link Staff}s to create new
 * {@link Event}s.
...
public class CreateEventCommand implements ICommand<Event> {
     ...
     * @param numTickets         number of initially available tickets for the
     *                           event. This can be 0 if the event does
     *                           not need booking.
     * @param ticketPriceInPence price in GBP pence per event ticket. This can be 0
     *                           if the event is free.
     * @param venueAddress       indicates where this performance will take place,
     *                           would be displayed to users in app
     * @param description        additional details about the event
     * @param startDateTime      indicates the date and time when this performance
     *                           is due to start
     * @param endDateTime        indicates the date and time when this performance
     *                           is due to end
     */
    ...
    /**
     * @return event number corresponding to the created event if successful and
     *         null otherwise
     */
    @Override
     ...
     * @verifies.that no other event with the same title has the same startDateTime
     *                and endDateTime
    ...
    public void execute(Context context, IView view) {
        ...
        if (numTickets <= 0) {
            view.displayFailure("CreateEventCommand",
                    LogStatus.CREATE_EVENT_INVALID_NUMBER_OF_TICKETS,
                    Map.of("numTickets", numTickets));
            eventResult = null;
            return;
        }
        ...
        if (!(tags == null || tags.getTags().isEmpty())) {
            for (String name : tags.getTags().keySet()) {
                if (!(possibleTagNames.contains(name))) {
                    view.displayFailure("CreateEventCommand",
                            LogStatus.CREATE_EVENT_TAG_NAME_NOT_KNOWN,
                            Map.of("tags", tags));
                    eventResult = null;
                    return;
                } else {
                    String tagValue = tags.getValueFor(name);
                    Set<String> possibleTagValues = possibleTags.get(name).getValues();
                    if (!(possibleTagValues.contains(tagValue))) {
                        view.displayFailure("CreateEventCommand",
                                LogStatus.CREATE_EVENT_TAG_VALUE_NOT_KNOWN,
                                Map.of("tags", tags));
                        eventResult = null;
                        return;
                    }
