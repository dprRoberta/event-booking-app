...
public class ListEventsCommand implements ICommand<List<Event>> {
    ...
    /**
     * @param userEventsOnly   if true, the returned events will be filtered
     *                         depending on the logged-in user:
     *                         for {@link Staff}s only the {@link Event}s they have
     *                         created,
     *                         and for {@link Consumer}s only the {@link Event}s
     *                         that match their preferences}
     * @param activeEventsOnly if true, returned {@link Event}s will be filtered to
     *                         contain only {@link Event}s with
     *                         {@link EventStatus#ACTIVE}
     * @param searchDate       chosen date to look for events. Can be null. If not
     *                         null, only {@link Event}s that are
     *                         happening on {@link #searchDate} (i.e., starting,
     *                         ending, or in between) will be included
     */
    ...
    protected static boolean eventSatisfiesPreferences(Map<String, EventTag> possibleTags,
            EventTagCollection preferences,
            Event event) {
        EventTagCollection eventTags = event.getTags();
        Set<String> possibleTagNames = possibleTags.keySet();
        boolean fitsPreferences = true;
        for (String name : possibleTagNames) {
            if (!(preferences.getTags().containsKey(name))) {
                // show any values for that tag
                fitsPreferences = fitsPreferences && true;
            } else if (!(eventTags.getTags().containsKey(name))) {
                // check default value
                fitsPreferences = fitsPreferences && preferences.getValueFor(name)
                        .equals(possibleTags.get(name).getDefaultValue());
            } else {
                // check that the preference matches the eventTag for the event
                fitsPreferences = fitsPreferences && preferences.getValueFor(name).equals(eventTags.getValueFor(name));
            }
        }
        return fitsPreferences;
    }
    ...
    protected static List<Event> filterEvents(List<Event> events, boolean activeEventsOnly, LocalDate searchDate) {
        ...
        if (searchDate != null) {
            filteredEvents = filteredEvents.filter(event -> event.getStartDateTime().toLocalDate().equals(searchDate)
