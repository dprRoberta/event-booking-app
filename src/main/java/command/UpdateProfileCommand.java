...
/**
 * {@link UpdateProfileCommand} contains common behaviour shared between profile
 * update commands
 */
public abstract class UpdateProfileCommand implements ICommand<Boolean> {
     ...
     * @param oldPassword password before the change, which must match the account's
     *                    password
     * @param newEmail    specified email address to use for the account after the
     *                    change
    ...
    /**
     * Common update method for profile changes that involve a change of email (used
     * as unique user identifier).
     * The method assumes error checking has already been performed before invoking
     * this.
     * 
     * @param context  object that provides access to global application state
    ...
    /**
     * @return True if successful, false if not, and null if the command has not
     *         been executed yet
     */
    @Override
