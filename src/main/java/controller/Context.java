...
/**
 * {@link Context} is a wrapper around the entire app state. It keeps references
 * to the internal states:
 * {@link IUserState}, {@link IEventState}, and {@link IBookingState}.
 * The state classes are kept as interfaces, so that other classes using the
 * context cannot depend on their
...
public class Context implements AutoCloseable, Serializable {
    ...
    private final OfflineMapSystem mapSystem;

    /**
     * Initialises all the state members with default constructors of the concrete
     * implementations:
     * {@link UserState}, {@link EventState}, and {@link BookingState}.
     *
     * @param orgName    Name of the organisation that owns this application
     *                   instance
     * @param orgAddress Address of the organisation that owns this application
     *                   instance
     * @param orgEmail   Email address of the organisation, used for payment
     *                   transactions
     * @param orgSecret  Secret passcode only known to organisation staff, required
     *                   for registering new Staff accounts
     */
    ...
    public Context(Context other) {
        ...
        mapSystem = new OfflineMapSystem((OfflineMapSystem) other.mapSystem);
    }
    ...
    public OfflineMapSystem getMapSystem() {
        return mapSystem;
    }
