...
public class BookEventSystemTests extends ConsoleTest {
    ...
    @Test
    void bookNonActiveEvent() {
        Controller controller = createStaffAndEvent(5, 1);
        List<Event> events = getAllEvents(controller);
        long firstEventNumber = events.get(0).getEventNumber();

        controller.runCommand(new CancelEventCommand(firstEventNumber, "Cancelling this event"));
        controller.runCommand(new LogoutCommand());
    ...
    void bookLessThanOneTicket() {
        ...
        startOutputCapture();
        controller.runCommand(new BookEventCommand(firstEventNumber, 0));
        stopOutputCaptureAndCompare(
